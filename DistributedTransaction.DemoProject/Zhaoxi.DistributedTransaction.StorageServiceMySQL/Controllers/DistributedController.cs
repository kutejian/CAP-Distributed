...
namespace Zhaoxi.DistributedTransaction.StorageServiceMySQL.Controllers
{
    ...
    public class DistributedController : ControllerBase
    {
        ...
        public void Distributed(User u, [FromCap] CapHeader header)
        {
            try
            {
                ...
                using (var trans = this._UserServiceDbContext.Database.BeginTransaction(publisher: this._iCapPublisher, autoCommit: false))
                {
